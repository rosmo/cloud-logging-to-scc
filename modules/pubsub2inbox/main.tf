...
resource "google_secret_manager_secret_version" "config-secret-version" {
  ...
  secret_data = var.config_contents != "" ? var.config_contents : file(var.config_file)
}
...
locals {
  ...
  iam_permissions = {
    scc-findings = {
      org     = ["roles/browser", "roles/securitycenter.findingsEditor"]
      project = []
      apis    = ["cloudresourcemanager.googleapis.com", "securitycenter.googleapis.com"]
    }
    scc = {
      org     = ["roles/browser"]
      project = []
...
locals {
  ...
  all_function_files   = setunion([for glob in local.function_files : fileset(format("%s/source/", path.module), glob)]...)
  function_file_hashes = [for file_path in local.all_function_files : filemd5(format("%s/source/%s", path.module, file_path))]
}
...
data "archive_file" "function-zip" {
  ...
  dynamic "source" {
    ...
    content {
      content  = file(format("%s/source/%s", path.module, source.value))
